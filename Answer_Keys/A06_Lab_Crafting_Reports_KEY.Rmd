...
```{r project.setup, echo=TRUE, error=FALSE, message=FALSE}
#Import libraries
...
>
> -   If your output table spans across two pages, try inserting a page break (via `\newpage`) in the markdown just before your code chunk.
...
```{r task3.1, echo=F, fig.align='left', fig.cap="Monthly Ozone, 2018", out.height="40%"}
epa_data %>% 
  filter(Year == '2018') %>% 
  drop_na(Ozone) %>% 
  ggplot(aes(
...
```{r task3.2,echo=F, fig.align='left', fig.cap="Monthly Ozone, 2019", out.height="40%"}
epa_data %>% 
  filter(Year == '2019') %>% 
  drop_na(Ozone) %>% 
  ggplot(aes(
...
## Git Repository
<https://github.com/ENV872/EDE_Fall2023>
